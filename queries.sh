...
echo "$($PSQL "SELECT t.name FROM teams AS t JOIN games AS g ON t.team_id = g.winner_id WHERE g.year=2018 AND g.round='Final'")"
...
echo "$($PSQL "SELECT g.year,t.name FROM games g LEFT JOIN teams t ON g.winner_id=t.team_id WHERE round='Final' ORDER BY t.name DESC")"
